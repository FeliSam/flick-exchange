    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        {notifications.some(n => !n.read) && (
          <button onClick={() => notifications.filter(n => !n.read).forEach(n => markNotificationRead(n.id))} style={{ background: 'none', border: 'none', color: 'var(--primary)', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>Tout lire</button>
        )}
      </div>
      {notifications.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {notifications.map(n => (
            <div key={n.id} onClick={() => markNotificationRead(n.id)} style={{ background: 'white', padding: '16px', borderRadius: '14px', borderLeft: `4px solid ${n.read ? 'var(--border)' : 'var(--primary)'}`, boxShadow: '0 2px 8px rgba(0,0,0,0.04)', cursor: 'pointer', transition: 'all 0.2s' }}>
