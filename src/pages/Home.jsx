  ...
  return (
    <div>
      ...
      <div className="card" style={{ padding: '16px' }}>
        <div className="card-title" style={{ marginBottom: '16px' }}>Actions rapides</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
          <button 
            className="btn btn-primary" 
            onClick={() => navigate('/transfer', { state: { direction: 'BJ_TO_RU' } })}
            style={{ flexDirection: 'column', padding: '24px 12px', borderRadius: '14px' }}
          >
            <div style={{ 
              width: '48px', height: '48px', borderRadius: '14px',
          ...
          <button 
            ...
            onClick={() => navigate('/transfer', { state: { direction: 'RU_TO_BJ' } })}
            style={{ flexDirection: 'column', padding: '24px 12px', borderRadius: '14px' }}
          >
            <div style={{ 
              width: '48px', height: '48px', borderRadius: '14px',
        ...
        <button 
          ...
          style={{ width: '100%', padding: '16px', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}
        >
          <FiRepeat size={20} />
          <span>Calculateur public</span>
      ...
      {pendingCount > 0 && (
        <div onClick={() => navigate('/history')} style={{ 
          ...
          color: '#92400E', cursor: 'pointer', display: 'flex',
          alignItems: 'center', gap: '12px'
        }}>
          <div style={{ fontSize: '1.5rem' }}>⏳</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>
              {pendingCount} transfert{pendingCount > 1 ? 's' : ''} en attente
            </div>
            <div style={{ fontSize: '0.8rem', opacity: 0.9, marginTop: '2px' }}>
              Finalisez le paiement pour lancer le transfert
            </div>
          </div>
          <FiArrowRight />
        </div>
      ...
      <div className="card" style={{ padding: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <button onClick={() => navigate('/history')} style={{ background: 'none', border: 'none', color: 'var(--primary)', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>
            Voir tout →
          </button>
        </div>
        ...
        {transfers.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '30px 20px', color: 'var(--text-muted)' }}>
            ...
            <button className="btn btn-primary" style={{ marginTop: '16px', maxWidth: '200px', margin: '16px auto 0' }} onClick={() => navigate('/transfer')}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {transfers.map(t => (
              <div key={t.id} onClick={() => navigate('/history')} style={{ 
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text)' }}>
                    {t.amountSent.toLocaleString()} {t.currencyFrom}
                  </div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                    → {t.amountReceived.toLocaleString()} {t.currencyTo} • {t.recipient.fullName}
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                    {formatDistanceToNow(new Date(t.createdAt), { locale: fr, addSuffix: true })}
                  </div>
                </div>
              </div>
            ))}
      ...
      <div style={{ 
        ...
        padding: '16px', borderRadius: '16px', marginTop: '8px',
        border: '1px solid #A7F3D0', display: 'flex', alignItems: 'center', gap: '12px'
      }}>
        <div style={{ 
        ...
        <div>
          ...
          <p style={{ fontSize: '0.8rem', color: '#047857', lineHeight: 1.5, marginTop: '2px' }}>
            Vos transactions sont vérifiées manuellement sous 2h ouvrables.
          </p>
        </div>
      </div>
    </div>
