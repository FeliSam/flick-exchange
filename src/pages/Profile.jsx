...
import { FiLogOut, FiUser, FiPhone, FiMail, FiMapPin, FiShield, FiCheckCircle, FiUpload, FiCamera, FiInfo, FiChevronRight } from 'react-icons/fi';
    ...
    <div>
      ...
      <div className="card" style={{ textAlign: 'center', background: 'linear-gradient(135deg, var(--primary), var(--primary-dark))', color: 'white', border: 'none', padding: '32px 20px' }}>
        <div style={{ 
          ...
          margin: '0 auto 16px', fontSize: '2.2rem', fontWeight: 800,
        ...
        <div onClick={() => setShowVerify(true)} style={{ 
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: '0.8rem', opacity: 0.9, marginTop: '2px' }}>
              Gagnez la confiance des autres utilisateurs
            </div>
          </div>
          <FiChevronRight />
        </div>
      ...
      {showVerify && (
        <div className="card animate-slide-up">
          ...
          {!uploaded ? (
            <>
              <p style={{ fontSize: '0.9rem', color: 'var(--text-light)', lineHeight: 1.6, marginBottom: '16px' }}>
                Pour gagner le badge "Identité vérifiée", veuillez télécharger une photo de votre passeport ou pièce d'identité.
              </p>
              <div style={{ 
                border: '2px dashed var(--primary)', borderRadius: '14px',
                padding: '40px 20px', textAlign: 'center', marginBottom: '16px',
                background: 'rgba(79,70,229,0.03)', cursor: 'pointer'
              }}>
            ...
            <div style={{ textAlign: 'center', padding: '20px' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '8px' }}>
                Votre compte est maintenant certifié.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          <div style={{ 
            width: '44px', height: '44px', borderRadius: '12px', 
            background: user.phoneNetwork === 'MTN' ? '#FFCC20' : user.phoneNetwork === 'Moov' ? '#0066CC' : '#00A651',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
        <div className="card" style={{ textAlign: 'center', padding: '20px' }}>
          <div style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--primary)' }}>
            {JSON.parse(localStorage.getItem('transfers') || '[]').filter(t => t.userId === user.id).length}
          </div>
        ...
        <div className="card" style={{ textAlign: 'center', padding: '20px' }}>
          <div style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--success)' }}>
            {JSON.parse(localStorage.getItem('transfers') || '[]').filter(t => t.userId === user.id && t.status === 'completed').length}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '2px' }}>
              Version {APP_VERSION} • Mis à jour Mai 2026
            </div>
          </div>
        </div>
      </div>
